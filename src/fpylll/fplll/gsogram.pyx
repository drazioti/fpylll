...
A ``MatGSOGram`` object stores the following information:
...
from fplll cimport MatGSOGram as MatGSOGram_c, Z_NR, FP_NR, Matrix
...
class MatGSOGramRowOpContext(object):
    """
...
cdef class MatGSOGram:
    """
    MatGSOGram provides an interface for performing elementary operations on a basis and computing its
    ...
    def __init__(self, IntegerMatrix B, U=None, UinvT=None,
        ...
        if B._type == ZT_MPZ:
            if float_type_ == FT_DOUBLE:
                ...
                self._core.mpz_d = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[d_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
            elif float_type_ == FT_LONG_DOUBLE:
                IF HAVE_LONG_DOUBLE:
                    self._type = gso_mpz_ld
                    self._core.mpz_ld = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[ld_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
                ELSE:
                    raise ValueError("Float type '%s' not understood." % float_type)
            elif float_type_ == FT_DPE:
                self._type = gso_mpz_dpe
                self._core.mpz_dpe = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[dpe_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
            elif float_type_ == FT_MPFR:
                self._type = gso_mpz_mpfr
                self._core.mpz_mpfr = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[mpfr_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
            else:
                IF HAVE_QD:
                    if float_type_ == FT_DD:
                        self._type = gso_mpz_dd
                        self._core.mpz_dd = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[dd_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
                    elif float_type_ == FT_QD:
                        self._type = gso_mpz_qd
                        self._core.mpz_qd = new MatGSOGram_c[Z_NR[mpz_t],FP_NR[qd_t]](b_m[0], u_m[0], u_inv_t_m[0], flags)
                    else:
                        raise ValueError("Float type '%s' not understood."%float_type)
                ELSE:
                    raise ValueError("Float type '%s' not understood."%float_type)
            ...
            if float_type_ == FT_DOUBLE:
                ...
                self._core.long_d = new MatGSOGram_c[Z_NR[long],FP_NR[d_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
            elif float_type_ == FT_LONG_DOUBLE:
                IF HAVE_LONG_DOUBLE:
                    self._type = gso_long_ld
                    self._core.long_ld = new MatGSOGram_c[Z_NR[long],FP_NR[ld_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
                ELSE:
                    raise ValueError("Float type '%s' not understood." % float_type)
            elif float_type_ == FT_DPE:
                self._type = gso_long_dpe
                self._core.long_dpe = new MatGSOGram_c[Z_NR[long],FP_NR[dpe_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
            elif float_type_ == FT_MPFR:
                self._type = gso_long_mpfr
                self._core.long_mpfr = new MatGSOGram_c[Z_NR[long],FP_NR[mpfr_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
            else:
                IF HAVE_QD:
                    if float_type_ == FT_DD:
                        self._type = gso_long_dd
                        self._core.long_dd = new MatGSOGram_c[Z_NR[long],FP_NR[dd_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
                    elif float_type_ == FT_QD:
                        self._type = gso_long_qd
                        self._core.long_qd = new MatGSOGram_c[Z_NR[long],FP_NR[qd_t]](b_l[0], u_l[0], u_inv_t_l[0], flags)
                    else:
                        raise ValueError("Float type '%s' not understood."%float_type)
                ELSE:
                    raise ValueError("Float type '%s' not understood."%float_type)
    ...
    def float_type(self):
        ...
        raise RuntimeError("MatGSOGram object '%s' has no core."%self)
    ...
    def row_op_begin(self, int first, int last):
        ...
        .. note:: It is preferable to use ``MatGSOGramRowOpContext`` via ``row_ops``.
        """
        if self._type == gso_mpz_d:
    ...
    def row_ops(self, int first, int last):
        ...
        """
        return MatGSOGramRowOpContext(self, first, last)
